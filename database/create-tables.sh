...
PGPASSWORD=$PASSWORD psql -U $USER -d $DATABASE -c "CREATE TABLE player (player_id serial primary key, USERname character varying, health integer, score integer, armor integer, survival_time integer, kill_count integer, is_dead boolean, room_id integer, attack_last_used character varying);"
